...
import { connectMultiplayer } from './multiplayerClient.js';
...
export function startGameEngine() {
  ...
  function createRemotePlayer(remote) {
    const group = new THREE.Group();
    const material = new THREE.MeshStandardMaterial({ color: remote.color || '#4dc9ff', roughness: 0.7 });
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.9, 1.5, 0.55), material);
    body.position.y = 0.8;
    ...
    group.add(head);
    group.position.set(remote.x, getHeight(remote.x, remote.z), remote.z);
    group.rotation.y = remote.yaw || 0;
    group.userData.target = new THREE.Vector3(group.position.x, group.position.y, group.position.z);
    group.userData.targetYaw = remote.yaw || 0;
    scene.add(group);
    remotePlayers.set(remote.id, group);
  }

  function updateRemotePlayer(remote) {
    if (remote.id === multiplayerSelfId) return;
    let group = remotePlayers.get(remote.id);
    if (!group) {
      createRemotePlayer(remote);
      group = remotePlayers.get(remote.id);
    }
    group.userData.target.set(remote.x, getHeight(remote.x, remote.z), remote.z);
    group.userData.targetYaw = remote.yaw || 0;
  }
  ...
  function setMultiplayerStatus(status) {
    const indicator = document.getElementById('multiplayer-status');
    if (!indicator) return;
    const labels = { connecting: 'CONNECTING...', online: 'ONLINE', offline: 'OFFLINE' };
  ...
  function setupMultiplayer() {
    ...
    indicator.id = 'multiplayer-status';
    indicator.dataset.status = 'connecting';
    indicator.textContent = '● CONNECTING...';
    Object.assign(indicator.style, {
      position: 'fixed', top: '18px', right: '22px', zIndex: '20',
      padding: '7px 11px', border: '1px solid rgba(255,255,255,.2)',
      borderRadius: '999px', background: 'rgba(7, 16, 29, .76)',
      color: '#9fffee', font: '600 11px/1.2 monospace', letterSpacing: '.08em'
    });
    document.body.appendChild(indicator);
    multiplayer = connectMultiplayer({
      onStatus: setMultiplayerStatus,
      onWelcome: ({ self, players }) => {
        multiplayerSelfId = self.id;
        players.filter((remote) => remote.id !== multiplayerSelfId).forEach(updateRemotePlayer);
      },
      onPlayerJoined: updateRemotePlayer,
      onPlayerState: updateRemotePlayer,
      onPlayerLeft: removeRemotePlayer
    });
  }

  function updateMultiplayer(dt) {
    for (const group of remotePlayers.values()) {
      group.position.lerp(group.userData.target, Math.min(dt * 12, 1));
      const rotationDelta = Math.atan2(
        Math.sin(group.userData.targetYaw - group.rotation.y),
        Math.cos(group.userData.targetYaw - group.rotation.y)
      );
      group.rotation.y += rotationDelta * Math.min(dt * 12, 1);
    }
    if (!multiplayer || !game.active) return;
    multiplayerSendTimer -= dt;
    if (multiplayerSendTimer <= 0) {
      multiplayerSendTimer = 0.05;
      multiplayer.sendState({ x: player.position.x, y: player.position.y, z: player.position.z, yaw: player.yaw });
    }
  }
